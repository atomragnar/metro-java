        ...
        while (programRunning) {
            ...
            if (inputCommands.isValid()) {
                switch (inputCommands.getCommand()) {
                    case APPEND -> metro.addStation(inputCommands.getLineName(),
                            inputCommands.getStationName(), 0, false);
                    case ADD_HEAD -> metro.addStation(inputCommands.getLineName(),
                            inputCommands.getStationName(), 0, true);
                    case ADD -> metro.addStation(inputCommands.getLineName(),
                            inputCommands.getStationName(), inputCommands.getTime(), false);
                    case REMOVE -> metro.removeStation(inputCommands.getLineName(), inputCommands.getStationName());
                    case OUTPUT -> metro.getMetroLine(inputCommands.getLineName())
                            .ifPresent(LinePrinter::printLine);
                    case CONNECT -> metro.connectStations(inputCommands.getLineName(),
                            inputCommands.getStationName(), inputCommands.getLineName2(),
                            inputCommands.getStationName2());
                    case ROUTE -> {
                        metro.route(inputCommands.getLineName(), inputCommands.getStationName(),
                                        inputCommands.getLineName2(), inputCommands.getStationName2())
                                .ifPresentOrElse(
